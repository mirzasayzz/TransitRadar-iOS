...
struct TransitRadarWidgetEntry: TimelineEntry {
    let date: Date
...
struct TransitRadarWidgetProvider: AppIntentTimelineProvider {

    func placeholder(in context: Context) -> TransitRadarWidgetEntry {
        TransitRadarWidgetEntry(
            date: Date(),
            activeShuttles: [],
    ...
    func snapshot(for configuration: TargetStopIntent, in context: Context) async -> TransitRadarWidgetEntry {
        let stop = configuration.stop ?? ShuttleStop.defaultStop
        return TransitRadarWidgetEntry(
            date: Date(),
            activeShuttles: [],
    ...
    func timeline(for configuration: TargetStopIntent, in context: Context) async -> Timeline<TransitRadarWidgetEntry> {
        ...
        do {
            ...
            let entry = TransitRadarWidgetEntry(
                date: Date(),
            ...
            let entry = TransitRadarWidgetEntry(date: Date(), activeShuttles: [], nextScheduledArrival: nil, groupedETAs: [], targetStop: targetStop)
...
struct TransitRadarWidgetEntryView: View {
    var entry: TransitRadarWidgetProvider.Entry
...
@main
struct TransitRadarWidget: Widget {
    let kind: String = "TransitRadarWidget"

    var body: some WidgetConfiguration {
        AppIntentConfiguration(kind: kind, intent: TargetStopIntent.self, provider: TransitRadarWidgetProvider()) { entry in
            TransitRadarWidgetEntryView(entry: entry)
        }
        .configurationDisplayName("TransitRadar-iOS")
        .description("TransitRadar-iOS Widget")
