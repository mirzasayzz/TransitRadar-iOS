...
private let logger = Logger(subsystem: "com.transitradar.ios", category: "APIClient")
...
struct APIClient {
    ...
    enum APIError: Error, LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .badStatus(let code):
                return "Server returned bad status code: \(code)."
    ...
    private func backgroundRetry<T: Decodable>(
        ...
        for attempt in 1...maxRetries {
            ...
            if Task.isCancelled {
                logger.debug("Background retry cancelled for \(endpoint.path)")
                return
            }
            ...
            do {
                ...
                logger.debug("Background retry cancelled during sleep for \(endpoint.path)")
                return
            }
            ...
            do {
                ...
                logger.info("Background retry succeeded on attempt \(attempt) for \(endpoint.path)")
                return
            } catch {
                logger.warning("Retry attempt \(attempt) failed for \(endpoint.path)")
                delay *= 2  // 2s -> 4s -> 8s
            }
        }
        logger.error("All background retries exhausted for \(endpoint.path)")
    }
}
